...
(function() {
	...
	Reader.prototype.readBlob = function(length, position, type, callback) { length = length-4;
	...
	DataView.prototype.getString = function(length, offset, raw) {
		...
		if(typeof Buffer !== 'undefined') {
			...
			throw new Error('File name decode false!');
			for(var i = offset; i < (offset + length); i++) {
	...
	RarArchive.prototype.get = function(entry, callback) {
		...
		this.rd.readBlob(entry.blockSize - 1, entry.offset + entry.headerSize, function(err, data) {
			...
			console.log("if(callback) callback.call(self, null, data)/data",  data)
